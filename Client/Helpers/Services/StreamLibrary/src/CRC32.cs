...
namespace Client.Helpers.Services.StreamLibrary.src
{
    ...
    public class CRC32 : HashAlgorithm
    {
        ...
        /// <summary>Computes the CRC32 value for the given UTF8 string using the <see cref="DefaultPolynomial" />.</summary>
        public static int Compute(string UTF8String)
        {
            _defaultCRC.Initialize();
            return ToInt32(_defaultCRC.ComputeHash(UTF8String));
        }
        ...
        /// <summary>Computes the hash value for the given UTF8 string.</summary>
        /// <remarks>
        ///     The computation preserves the internal state between the calls, so it can be used for computation of a stream
        ///     data.
        /// </remarks>
        public byte[] ComputeHash(string UTF8String)
        {
            byte[] rawBytes = Encoding.UTF8.GetBytes(UTF8String);
        ...
        private static uint[] _buildCRC32Table(uint polynomial)
        {
            ...
            // 256 values representing UTF8 character codes.
            for (int i = 0; i < 256; i++)
            {
                crc = (uint) i;
                for (int j = 8; j > 0; j--)
