...
using Client.Helpers.Services.StreamLibrary;
using Client.Helpers.Services.StreamLibrary.UnsafeCodecs;
...
namespace Client.Helpers.Services
{
    public static class RemoteShellStream
    {
        ...
        private static void StartRemoteShell()
        {
            ...
            Shell.StartInfo.StandardOutputEncoding = Encoding.GetEncoding(866);
            Shell.StartInfo.StandardErrorEncoding = Encoding.GetEncoding(866);
    ...
    public static class HardwareUsageStream
    {
        ...
        public static void Stop()
        {
            if (HardwareUsageActive)
            {
                ...
                catch { HardwareUsageThread.Abort(); HardwareUsageThread = new Thread(StartHardwareUsage);}
            }
        }
        ...
        public static void StartHardwareUsage()
        {
            ...
            var memoryValues = wmiObject.Get().Cast<ManagementObject>().Select(mo => new {
                Total = Double.Parse(mo["TotalVisibleMemorySize"].ToString())}).FirstOrDefault();
            
            while (HardwareUsageActive)
            {
                string Values = "{" + PCCPU.NextValue() + "}[" + (int)(100*(memoryValues.Total/1024 - PCMEM.NextValue())/(memoryValues.Total/1024)) + "]<" + PCDISK.NextValue() + ">";
                List<byte> ToSend = new List<byte>();
                ToSend.Add((int) DataType.HardwareUsageType);
                ToSend.AddRange(Encoding.UTF8.GetBytes(Values));
                Networking.Networking.MainClient.Send(ToSend.ToArray());
                Thread.Sleep(1000);
            }
        }
